...
function renderNoticeTimelineItem(item: NoticeItem): string {
  return `
    <a data-route href="${escapeHtml(toNoticeDetailPath(item.id))}" data-anim="notice-item" class="premium-card group block relative rounded-2xl p-4 md:p-5 transition-colors">
...
function renderMobileNoticeItem(item: NoticeItem): string {
  return `
    <a data-route href="${escapeHtml(toNoticeDetailPath(item.id))}" class="premium-card group grid grid-cols-[96px_1fr] gap-3 rounded-2xl p-3 transition-colors">
...
function renderNotices(items: NoticeItem[]): string {
  ...
  return `
    <div class="mb-6 flex flex-wrap items-center gap-2 text-[11px] md:text-xs uppercase tracking-[0.12em]">
      <span class="premium-chip">${items.length} noticias</span>
      <span class="premium-chip">Ultima noticia em ${escapeHtml(latest.dataLabel)}${latest.horarioLabel ? ` • ${escapeHtml(latest.horarioLabel)}` : ''}</span>
      <span class="premium-chip bg-cor-primaria/90 text-cor-escura">Espaco cultural</span>
    </div>

    <div class="lg:hidden space-y-6">
      <a data-route href="${escapeHtml(toNoticeDetailPath(featured.id))}" data-anim="notice-featured" class="premium-card group block rounded-3xl overflow-hidden transition-colors">
        ...
        <div class="p-5">
          ...
          <p class="mt-3 text-cor-texto/80 text-sm leading-relaxed line-clamp-3">${escapeHtml(featured.resumo)}</p>
      ...
      ${items.length > 1 ? `
        <a data-route href="/noticias" class="premium-button-secondary w-full px-5 py-3 text-sm">
    ...
    <div class="hidden lg:grid lg:grid-cols-[1.05fr_0.95fr] gap-6 md:gap-8 items-start">
      ...
      <div data-anim="notice-list" class="relative md:pl-8">
        ...
        ${hiddenItemsCount > 0 ? `
          <a data-route href="/noticias" class="premium-button-secondary mt-5 w-full md:w-auto px-5 py-3 text-sm">
            Ver mais noticias
            <span>→</span>
          </a>
        ` : ''}
    ...
    <section id="noticias" data-anim="notices" class="relative py-20 md:py-32 bg-cor-fundo scroll-mt-24 md:scroll-mt-28">
      <div class="relative z-10 max-w-7xl mx-auto px-6">
        <div class="max-w-3xl mb-9 md:mb-16">
          ...
          <span data-anim="notices-kicker" class="section-kicker block">Ultimas atualizacoes</span>
          <h2 data-anim="notices-title" class="section-title mt-4 text-3xl sm:text-4xl md:text-5xl">TIMELINE DE NOTICIAS</h2>
          <p data-anim="notices-subtitle" class="section-copy mt-4 text-base md:text-lg max-w-2xl">Acompanhe nossas novidades, eventos e a rotina do espaco em ordem cronologica.</p>

...
export function MobileMenu() {
  return `
    <div id="mobile-menu" class="fixed inset-0 z-[120] invisible" aria-hidden="true">
      <!-- Overlay com blur -->
      ...
      <!-- Menu panel -->
      <div
        class="absolute inset-y-0 right-0 flex w-full max-w-sm flex-col overflow-y-auto bg-gradient-to-b from-cor-fundo to-cor-fundo/95 shadow-2xl transform translate-x-full transition-transform duration-500 ease-out"
        id="menu-panel"
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <!-- Links de navegação -->
        <nav class="flex-1 overflow-y-auto p-6 pb-4 space-y-3" aria-label="Links principais">
...
<div class="sticky bottom-0 mt-auto border-t border-cor-primaria/10 bg-cor-fundo/98 px-8 pt-5 md:pt-8 pb-[max(env(safe-area-inset-bottom),1rem)] md:pb-8">
